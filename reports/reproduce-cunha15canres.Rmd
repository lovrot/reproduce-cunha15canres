...
setdiff_tbl <- pData(qcsubstudy) %>%
  ...
  ## https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSM2178395 > Description
  rowwise() %>%
  mutate(set1002 = ifelse(setnr == 1022, "1022", "(rest)"))
```
...
ggplot_setdiff <- function(data) {
  ...
  ggplot(data, aes(x = original, y = reextract, shape = set1002)) +
    ...
    coord_fixed() +
    lims(x = lim0, y = lim0) +
    scale_shape_manual(values = c("1022" = 1, "(rest)" = 16), guide = FALSE) +
    labs(
