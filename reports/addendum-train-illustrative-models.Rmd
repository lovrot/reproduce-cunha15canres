...
Load/attach additional packages.
...
```{r}
casecontstudy$casecontcd <- factor(casecontstudy$casecontstat,
  levels = c(0, 1),
...
```{r}
qcsubstudy_primary_endpoint_inclusion <- pData(qcsubstudy) %>%
...
casecontsets_not_in_qcsubstudy <- 
  setdiff(all_casecontsets, qcsubstudy_primary_endpoint_inclusion$setnr)
...
```{r}
save(qcsubstudy_primary_endpoint_inclusion, testing, training, fit_list,
